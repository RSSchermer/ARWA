...
use crate::{GenericNode, InvalidCast, InvalidPointerId, Node, PointerId, QuerySelectorAll};
...
pub trait Element: AsRef<web_sys::Element> {
    ...
    fn query_selector_first(&self, selector: &str) -> Result<Option<GenericElement>, SyntaxError> {
        self.as_ref()
            ...
            .map(|ok| ok.map(|e| GenericElement::from(e)))
            .map_err(|err| SyntaxError::new(err.unchecked_into()))
    }
    ...
    fn previous_sibling_element(&self) -> Option<GenericElement> {
        self.as_ref()
            ...
            .map(|inner| GenericElement { inner })
    }
...
impl<'a> ChildElements<'a> {
    ...
    pub fn find_by_id(&self, id: &str) -> Option<GenericElement> {
        self.children.get_with_name(id).map(|inner| GenericElement {
            inner: inner.unchecked_into(),
        })
    }
...
/*
pub struct LiveQueriedElements {
    inner: web_sys::HtmlCollection,
}

impl LiveQueriedElements {
    pub fn get(&self, index: usize) -> Option<GenericElement> {
        u32::try_from(index)
            .ok()
    ...
    pub fn iter(&self) -> LiveQueriedElementsIter {
        LiveQueriedElementsIter {
            elements: self,
            current: 0,
        }
    }
...
impl IntoIterator for LiveQueriedElements {
    type Item = GenericElement;
    type IntoIter = LiveQueriedElementsIntoIter;

    fn into_iter(self) -> Self::IntoIter {
        LiveQueriedElementsIntoIter {
            elements: self,
            current: 0,
        }
    }
...
pub struct LiveQueriedElementsIter<'a> {
    elements: &'a LiveQueriedElements,
    current: usize,
}

impl<'a> Iterator for LiveQueriedElementsIter<'a> {
    type Item = GenericElement;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        self.elements.get(current)
    }
}

pub struct LiveQueriedElementsIntoIter {
    elements: LiveQueriedElements,
    current: usize,
}

impl Iterator for LiveQueriedElementsIntoIter {
    type Item = GenericElement;
