...
pub trait Node: AsRef<web_sys::Node> {
    ...
    fn root_node(&self) -> GenericNode {
        GenericNode {
            inner: self.as_ref().get_root_node(),
        }
    }
    ...
    fn root_node_composed(&self) -> GenericNode {
        ...
        GenericNode {
            inner: self.as_ref().get_root_node_with_options(&options),
        }
    }
    ...
    fn parent_node(&self) -> Option<GenericNode> {
        self.as_ref()
            .parent_node()
            .map(|inner| GenericNode { inner })
    }

    fn parent_element(&self) -> Option<GenericElement> {
        self.as_ref()
            .parent_element()
            .map(|element| GenericElement::from(element))
    }
...
impl<'a> ChildNodes<'a> {
    pub fn get(&self, index: usize) -> Option<GenericNode> {
        u32::try_from(index)
            .ok()
            .and_then(|index| self.children.get(index).map(|inner| GenericNode { inner }))
    }
