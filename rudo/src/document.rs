...
pub trait Document: AsRef<web_sys::Document> {
    ...
    fn body(&self) -> Option<HtmlBodyElement> {
        ...
        self.as_ref()
            ...
            .map(|body| HtmlBodyElement::from(body))
    }
    ...
    fn set_text_directionality(&self, text_directionality: TextDirectionality) {
        let text_directionality = match text_directionality {
    ...
    fn doctype(&self) -> Option<DocumentType> {
        self.as_ref().doctype().map(|d| DocumentType::from(d))
    }
    ...
    fn head(&self) -> Option<HtmlHeadElement> {
        self.as_ref().head().map(|h| HtmlHeadElement::from(h))
    }
    ...
    fn location(&self) -> Option<Location> {
        self.as_ref().location().map(|l| Location::from(l))
    }
    ...
    fn element_from_point(&self, x: f32, y: f32) -> Option<GenericElement> {
        self.as_ref().element_from_point(x, y).map(|e| GenericElement::from(e))
    }
}
